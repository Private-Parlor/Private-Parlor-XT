...
module PrivateParlorXT
  ...
  class DownvoteHandler < HearsHandler
    @downvote_limit_interval : Int32 = 0
    ...
    def do(ctx : Tourmaline::Context, relay : Relay, access : AuthorizedRanks, database : Database, history : History, locale : Locale, spam : SpamHandler?)
      ...
      if spam && spam.spammy_downvote?(user.id, @downvote_limit_interval)
      ...
      if user.id == reply_user.id
        return relay.send_to_user(message.message_id.to_i64, user.id, locale.replies.downvoted_own_message)
      end
      ...
      relay.send_to_user(message.message_id.to_i64, user.id, locale.replies.gave_downvote)

      unless reply_user.hide_karma
        relay.send_to_user(
          history.get_receiver_message(reply.message_id.to_i64, reply_user.id), 
          reply_user.id,
          locale.replies.got_downvote
        )
      end
    end
