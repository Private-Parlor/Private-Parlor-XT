...
module PrivateParlorXT
  ...
  class VideoHandler < UpdateHandler
    ...
    def do(update : Tourmaline::Context, relay : Relay, access : AuthorizedRanks, database : Database, history : History, locale : Locale, spam : SpamHandler?)
      ...
      unless access.authorized?(user.rank, :Video)
        response = Format.substitute_message(locale.replies.media_disabled, locale, {"type" => "video"})
      ...
      if spam && spam.spammy_video?(user.id)
