...
module PrivateParlorXT
  ...
  class DocumentHandler < UpdateHandler
    ...
    def do(update : Tourmaline::Context, relay : Relay, access : AuthorizedRanks, database : Database, history : History, locale : Locale, spam : SpamHandler?)
      ...
      unless access.authorized?(user.rank, :Document)
        response = Format.substitute_message(locale.replies.media_disabled, locale, {"type" => "document"})
      ...
      if spam && spam.spammy_document?(user.id)
