...
module PrivateParlorXT
  ...
  class VoiceHandler < UpdateHandler
    ...
    def do(update : Tourmaline::Context, relay : Relay, access : AuthorizedRanks, database : Database, history : History, locale : Locale, spam : SpamHandler?)
      ...
      unless access.authorized?(user.rank, :Voice)
        response = Format.substitute_message(locale.replies.media_disabled, locale, {"type" => "voice"})
      ...
      if spam && spam.spammy_voice?(user.id)
