...
module PrivateParlorXT
  describe StickerHandler do
    ranks = {
      10 => Rank.new(
        "Mod",
        Set(CommandPermissions).new,
        Set{
          MessagePermissions::Sticker,
        },
      ),
      -5 => Rank.new(
        "Restricted",
    ...
    describe "#do" do
      ...
      it "returns early if user is not authorized" do
        ...
        messages[0].data.should_not(eq("sticker_item_one"))
      end
      ...
      it "returns early if message has no sticker" do
        services = create_services(ranks: ranks, relay: MockRelay.new("", MockClient.new))
      ...
      it "returns early with 'unoriginal message' response if Robot9000 is enabled and message is not unique" do
        ...
        messages.size.should(eq(4))
        messages[0].data.should(eq("sticker_item_one"))
      ...
      it "records message statistics when statitics is enabled" do
        ...
        result[Statistics::MessageCounts::Stickers].should(eq(1))
      ...
      it "queues sticker" do
        ...
        messages.each do |msg|
          ...
          msg.data.should(eq("sticker_item_one"))
      ...
      it "queues sticker with reply" do
        services = create_services(ranks: ranks, relay: MockRelay.new("", MockClient.new))
    ...
    describe "#spamming?" do
      it "returns true if user is spamming stickers" do
        ...
        spam_services = create_services(
          spam: SpamHandler.new(spam_limit: 10, score_sticker: 6)
        )
    ...
    describe "#has_sufficient_karma?" do
      ...
      it "returns true if price for stickers is less than 0" do
        services = create_services(karma_economy: KarmaHandler.new(karma_sticker: -10))
    ...
    describe "#spend_karma" do
      ...
      it "returns unaltered user if price of stickers is less than 0" do
        services = create_services(karma_economy: KarmaHandler.new(karma_sticker: -10))
