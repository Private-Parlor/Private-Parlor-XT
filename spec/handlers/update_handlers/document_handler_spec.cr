  ...
  describe DocumentHandler do
    ...
    describe "#do" do
      it "returns early if message is a forward" do
        ...
        handler = DocumentHandler.new(MockConfig.new)
      ...
      it "returns early if message is an animation" do
        services = create_services(ranks: ranks, relay: MockRelay.new("", MockClient.new))
      ...
      it "returns early if user is not authorized" do
        ...
        messages[0].data.should_not(eq("document_item_one"))
      end
      ...
      it "returns early with 'spamming' response if user is spamming" do
        services = create_services(
          ...
          spam: SpamHandler.new(
            spam_limit: 10, score_document: 6
          ),
          relay: MockRelay.new("", MockClient.new))
      ...
      it "returns early if message has no document" do
        services = create_services(ranks: ranks, relay: MockRelay.new("", MockClient.new))
      ...
      it "returns early with 'unoriginal message' response if Robot9000 is enabled and message is not unique" do
        ...
        messages.size.should(eq(4))
        messages[0].data.should(eq("document_item_one"))
      ...
      it "queues document with reply" do
        services = create_services(ranks: ranks, relay: MockRelay.new("", MockClient.new))
    ...
    describe "#spamming?" do
      it "returns true if user is spamming documents" do
        services = create_services(ranks: ranks)
      ...
      it "returns false if user is not spamming documents" do
        services = create_services(ranks: ranks)
    ...
    describe "#has_sufficient_karma?" do
      ...
      it "returns true if price for documents is less than 0" do
        services = create_services(karma_economy: KarmaHandler.new(karma_document: -10))
    ...
    describe "#spend_karma" do
      ...
      it "returns unaltered user if price of documents is less than 0" do
        services = create_services(karma_economy: KarmaHandler.new(karma_document: -10))
